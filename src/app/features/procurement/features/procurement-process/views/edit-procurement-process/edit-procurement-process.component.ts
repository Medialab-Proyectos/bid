...
@Component({
  selector: 'fi-edit-procurement-process',
  templateUrl: './edit-procurement-process.component.html',
})
export class EditProcurementProcessComponent implements OnInit, OnDestroy {
  ...
  biddingProcessId = '';
  procurementProcess: ProcurementProcessDetails = null;
  procurementProcessDetail: BiddingProcessProcurementProcessDetail;
  ...
  formConfig: FormConfig = {
    ...
    mode: ModeOfProcurementForm.UPDATE,
  };
  ...
  showEditBtn() {
    return !this.biddingProcessFormSvc.isUnableToEditProcess(
      this.procurementProcessDetail.process.status,
      this.biddingProcessPlan.status
    );
  }
  ...
  getPlanStatus(): void {
    const sub = this.biddingStoreSvc
      ...
      .subscribe((state) => {
        this.biddingProcessPlan = state?.biddingPlanState?.biddingProcessPlan;
  ...
  checkSaveCommentAndCancelBtnVisibility(): void {
    ...
    if (this.isInternal) {
      this.displaySaveCommentAndCancelBtn =
        !statusesNotShowAddCommentOption.includes(this.processStatus);
    } else {
      this.displaySaveCommentAndCancelBtn = !(
        statusesNotShowAddCommentOption.includes(this.processStatus) ||
        externalRestriction.includes(this.processStatus)
      );
    }
  }
  ...
  loadEnums() {
    ...
    return this.enumStoreSvc
      .selectEnums()
      .pipe(
        filter((data) => requiredEnums.every((key) => data.enumsLoaded[key]))
      );
  }

  populateForm(): void {
    this.subscription.add(
      this.loadEnums().subscribe((data) => {
        ...
        this.biddingProcessFormSvc
          ...
          .subscribe((data) => {
            this.procurementProcessDetail = data;
            ...
            this.biddingProcessFormSvc.disableFormField(this.form, data);
            ...
            this.formConfig = this.biddingProcessFormSvc.configForm(data);
            this.formConfig.mode = ModeOfProcurementForm.UPDATE;
    ...
    this.subscription.add(
      this.enumStoreSvc.selectEnums().subscribe((data) => {
        if (
          ...
          this.procurementProcessRequest =
            this.biddingProcessFormSvc.procurementRequest(
              this.form,
              this.attributeCountry.value.toString(),
              data.biddingProcessProcurementProcessCategories,
              data.biddingProcessProcurementProcessProcurementMethods,
              data.biddingProcessProcurementProcessSupervisionMethods,
              data.biddingProcessMilestoneCodes,
          ...
          if (this.formErrorCollection.length === 0) {
            ...
            this.biddingProcessPlanSvc
              ...
              .subscribe(
                () => {
                  this.biddingProcessFormSvc.successEditMessage();
