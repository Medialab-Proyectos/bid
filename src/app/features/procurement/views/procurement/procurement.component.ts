...
import { WorkflowCommentsModalComponent } from '@fiduciary-interface/app/shared/components/workflow-comments-modal/workflow-comments-modal.component';
import { selectBiddingProcessPlan } from '@core/store/bidding-process-plan/selectors/bidding-process-plan.selector';
import { ProcurementProcessFormService } from '../../features/procurement-process/services/procurement-process-form.service';
import {
  hideLoader,
  showLoader,
} from '../../../../core/store/loader/loader.actions';

@Component({
  selector: 'fi-procurement',
  templateUrl: './procurement.component.html',
})
export class ProcurementComponent implements OnInit, OnChanges, OnDestroy {
  public navLinks = [
    {
      label: 'PROCUREMENT_COMMENTS_TAB',
      link: './procurement-comments',
      index: 0,
      status: 'active',
    },
  ];
  ...
  validState: boolean;
  mobileView: boolean;
  dialogNoObjectionResult: string;
  dialogUnofficialReviewResult: string;
  dialogOpened = true;
  ...
  public downloadRawDataPermission: PermissionEnum[] = [
    PermissionEnum.DOWNLOAD_RAW_DATA,
  ];

  public showCommentsTabPermission: PermissionEnum[] = [
    PermissionEnum.VIEW_PROCUREMENT_INFORMATION,
  ];
  public declareIneligibilityPermissionLevel0: PermissionEnum[] = [
    PermissionEnum.INELIGIBILITYDECLARATIONLEVEL0,
  ];
  public declareIneligibilityPermissionLevel3: PermissionEnum[] = [
    PermissionEnum.INELIGIBILITYDECLARATIONLEVEL3,
  ];

  selectedLanguage: string;
  isInternal: boolean;
  showDownloadRawDataBtn: boolean;
  ...
  constructor(
    ...
    private procurementProcessFormSvc: ProcurementProcessFormService
  ) {}
  ...
  setBiddingPlan(biddingPlanState: BiddingProcessPlanState): void {
    if (
      ...
      // Always use the full list to avoid being affected by external filters
      const processes: BiddingProcessProcurementProcess[] =
        biddingPlanState.biddingProcessProcurementProcesses;
      ...
      if (this.procurementPlanCollection.length > 0) {
        this.showDownloadRawDataBtn = true;
      }
  ...
  sentApprovalRequest() {
    this.sendingProcurementForApproval = true;

    // Unsubscribe from store just as a safety measure to prevent rogue subscriptions
    this.unsubscribeFromStore();

    const instance = this.dialogRef.content
      .instance as WorkflowCommentsModalComponent;

    // We disable the kendo editor so the user don't change the editor text while sending the approval
    instance.myForm.disable();

    // We listen to biddingProccessPlan state change so we close the modal once the requestApprovalAction has finished
    this.storeSubscription = this.store
      .select(selectBiddingProcessPlan)
      .pipe(skip(1))
      .subscribe((biddingProcessPlan) => {
        if (biddingProcessPlan.processScreenLoaded) {
          this.sendingProcurementForApproval = false;
          this.closeWorkflowCommentsModal();
        }
      });

    this.biddingProcessPlanStore.requestApprovalAction(
      {
        projectBucketId: this.projectSelected.projectBucketId,
        instAcronym: this.projectSelected.executorAcronym,
        entityTypeId: this.procurementId,
        isInternalVisibility: true,
        businessRulesRequest: {
          factors: {
            workflowSection: WorkflowEntityScreen.PROCUREMENT_PLAN,
          },
        },
        workflowComment: {
          text: instance.myForm.value.editor,
          visibility: true,
          status: WorkflowCommentStatusEnum.COMPLETED,
        },
      },
      this.projectSelected.contract,
  ...
  checkIneligibilityOption(process: BiddingProcessProcurementProcess): boolean {
    if (process.packagesUnderReview) return false;
    if (
      process.category.name === CategoryProcurement.PROCT_INDCST ||
      process.procurementMethod.name === ProcurementMethodCode.PROCT_SRQOI ||
      process.procurementMethod.name === ProcurementMethodCode.PROCT_SRMQ
    ) {
      return this.permissionSvc.haveSomePermissions(
        this.declareIneligibilityPermissionLevel0
      );
    } else {
      return this.permissionSvc.haveSomePermissions(
        this.declareIneligibilityPermissionLevel3
      );
    }
  }
  ...
  setCurdActions(
    ...
    const ineligibilityeOption = ProcessActions.ineligibility;
    ...
    switch (process.status) {
      ...
      case BiddingProcessProcurementProcessStatuses.CONTRACT_SIGNED:
        if (
          planStatus !== BiddingProcessPlanStatus.IN_SYNC &&
          this.checkIneligibilityOption(process)
        ) {
          output.push(ineligibilityeOption);
        }
        break;
    }
    ...
    if (
      this.procurementProcessFormSvc.isUnableToEditProcess(
        process.status,
        this.planStatus
      )
    ) {
      output = output.filter((action) => action !== ProcessActions.edit);
    }
    return output;
  }
  ...
  openCommentsModalLogic(): void {
    this.isCommentsLoading = true;
    this.commentsSvc
      .getComments(this.processPlan.id, CommentsDomain.BIDDINGPROCESSPLAN)
      .subscribe(
        (comments: ProcurementCommentGetResponse) => {
          this.commentsList = this.sortCommentsByDate(comments.comments);
          this.openCommentModalAction(this.planStatus);
        },
        () => {
          const message = this.translate.instant(
            'COMMENTS.ERROR_LOADING_COMMENTS'
          );
          this.showErrorMsg(message);
        }
      )
      .add(() => (this.isCommentsLoading = false));
  }
  ...
  openCommentModalAction(planStatus: BiddingProcessPlanStatus): void {
    this.openCommentsModal(planStatus).subscribe((data: DialogResponse) => {
      if (data.result === ModalOptions.ACCEPT) {
        const comments: ProcurementCommentRequest[] = [];
        data.content.generalForm.get('comments').controls.forEach((c) => {
          const comment: ProcurementCommentRequest = {
            text: c.get('text').getRawValue(),
            visibility: Number(c.get('visibility').getRawValue()),
            id: c.get('id').getRawValue(),
          };
          comments.push(comment);
        });
        this.postComment(this.cleanComments(comments));
      }
    });
  }
  ...
  cleanComments(
    ...
    if (comments.length === 1 && comments[0].text === '') {
      return [];
    } else {
      return comments;
    }
    ...
    this.subscriptionCollection.push(
      combineLatest([
        this.storeProject.selectedProject().pipe(
          ...
          distinctUntilChanged(
            (a, b) => a.selectedProject.id === b.selectedProject.id
          )
        ),
        this.store.select(selectBiddingProcessPlan).pipe(
          filter((data) => !!data.biddingProcessPlan),
          distinctUntilChanged(
            (a, b) => a.biddingProcessPlan.id === b.biddingProcessPlan.id
          )
        ),
      ]).subscribe(([projectData, planState]) => {
        this.workflowSharedSvc.loadActions({
          body: {
            entityTypeId: planState.biddingProcessPlan.id,
            projectBucketId: projectData.selectedProject.projectBucketId,
            idEntityType: WorkflowIdEntityType.PROCUREMENT_PLAN,
          },
          projectContractId: projectData.selectedProject.contract,
          instAcronym: projectData.selectedProject.executorAcronym,
        });
      })
    );
  }
  ...
  public onSelect(e: SelectEvent): void {
    this.tabEvents.push(e);
    const lengthEvents = this.tabEvents.length;

    if (
      this.tabEvents.length >= 2 &&
      this.tabEvents[lengthEvents - 2].index !==
        this.tabEvents[lengthEvents - 1].index &&
      e.index === 1
    ) {
      this.reloadPlans = !this.reloadPlans;
    }
  }
      ...
      .pipe(
        ...
        switchMap(() => {
          return this.notificationService.sendNotification({
            EntityType: WorkflowIdEntityType.PROCUREMENT_PLAN,
  ...
  downloadRawData(): void {
    ...
    const sub = this.procurementDownload
      .downloadRawData(this.procurementId)
      .subscribe({
        next: (res) => {
          const docName = `Raw Data - ${this.contractNumber}.xlsx`;
          this.saveDocument(res.body, docName);
          const message = this.translate.instant(
            'PROCUREMENT.RAW_DATA.SUCCESS'
          );
          this.notificationGlobalSvc.showSuccess(message);
        },
        error: () => {
          this.store.dispatch(hideLoader());
          const message = this.translate.instant('PROCUREMENT.RAW_DATA.ERROR');
          this.notificationGlobalSvc.showError(message);
        },
        complete: () => {
          this.store.dispatch(hideLoader());
  ...
  getContractNumber(): void {
    const sub = this.storeProject.selectedProject().subscribe((data) => {
