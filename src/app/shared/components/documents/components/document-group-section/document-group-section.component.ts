...
@Component({
  selector: 'fi-document-group-section',
  templateUrl: './document-group-section.component.html',
})
export class DocumentGroupSectionComponent implements OnDestroy {
  ...
  onDeleteFile(event: EventFileDelete) {
    ...
    for (const group of this.groups) {
      ...
      if (document) {
        ...
        this.documentApi
          .deleteGeneralProcurementDocument(documentId, this.domain)
          .subscribe(
            () => {
  ...
  onEditFile(data: EventFileEdit): void {
    ...
    if (event.item.needBeUploaded) {
      this.uploadDocument(groupId, event);
    } else {
      this.changeGroupDocument(groupId, event);
    }
  }

  uploadDocument(groupId: string, event: EventDocument): void {
    this.documentApi
  ...
  changeGroupDocument(groupId: string, event: EventDocument): void {
    ...
    this.documentApi
      .changeGroupOfDocument(event.item.id, groupId, this.domain)
      .subscribe(
        () => {
          ...
          newGroup.fiduciaryProcessDocuments.push(event.item);
